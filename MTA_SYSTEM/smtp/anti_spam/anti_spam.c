...
static BOOL anti_spam_register_statistic(STATISTIC_FUNCTION func);

static BOOL anti_spam_unregister_statistic(STATISTIC_FUNCTION func);

static BOOL anti_spam_register_filter(char* type, FILTER_FUNCTION func);

static BOOL anti_spam_unregister_filter(char* type, FILTER_FUNCTION func);

static BOOL anti_spam_register_auditor(AUDITOR_FUNCTION func);

static BOOL anti_spam_unregister_auditor(AUDITOR_FUNCTION func);

static BOOL anti_spam_register_judge(JUDGE_FUNCTION func);

static BOOL anti_spam_unregister_judge(JUDGE_FUNCTION func);

static BOOL anti_spam_register_talk(TALK_MAIN talk);

static BOOL anti_spam_unregister_talk(TALK_MAIN talk);

static int anti_spam_getversion();

static void* anti_spam_queryservice(char *service);

static void anti_spam_reset_envelop_files(ENVELOP_INFO *penvelop);

static void anti_spam_reset_head_files(MAIL_HEAD *phead);

static void anti_spam_reset_body_files(MAIL_BODY *pbody);

static CONNECTION* get_connection_by_id(int context_ID);

static MAIL_ENTITY get_mail_entity_by_id(int context_ID);

static BOOL anti_spam_set_extra_value(int context_ID, char* tag,char* pval);

static void anti_spam_mark_context_spam(int context_ID);

static BOOL anti_spam_is_need_auth();

static BOOL anti_spam_is_domainlist_valid();

static const char* anti_spam_get_default_domain();

static const char* anti_spam_get_plugin_name();

static const char* anti_spam_get_config_path();

static const char *anti_spam_get_data_path();

static int anti_spam_get_context_num();
...
/*
 *	  anti-spam's construct function
 *	  @param
...
{	 
	...
	if (NULL == dirp){
		printf("[anti_spam]: fail to open plugins' directory %s\n",
				g_init_path);
		return -1;
	}
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		if (strcmp(ext_name, ".pas") == 0){
			...
			anti_spam_load_library(temp_path);
		}
	}
	closedir(dirp);
	return 0;
...
/*
 *	  load a library into anti-spam module
 *	  @param	
 *		  path	  indicate the file name
 *	  @return	  
 *		  PLUGIN_LOAD_OK			  success
 *		  PLUGIN_ALREADY_LOADED		  already loaded by anti-spam module
...
int anti_spam_load_library(const char* path)
{
	...
	two_server_funcs[0] = (void*)anti_spam_getversion;
	two_server_funcs[1] = (void*)anti_spam_queryservice;
	...
	for (pnode=double_list_get_head(&g_list_lib); NULL!=pnode;
		 pnode=double_list_get_after(&g_list_lib, pnode)) {
			if (strcmp(((SHARELIB*)(pnode->pdata))->file_name, pname) == 0) {
				printf("[anti_spam]: %s is already loaded by anti-spam"
						" module\n", pname);
	...
	if (NULL == handle){
		printf("[anti_spam]: error to load %s reason: %s\n", fake_path, 
				dlerror());
		printf("[anti_spam]: the plugin %s is not loaded\n", fake_path);
		return PLUGIN_FAIL_OPEN;
	}
	func = (PLUGIN_MAIN)dlsym(handle, "AS_LibMain");
	if (NULL == func) {
		printf("[anti_spam]: error to find AS_LibMain function in %s\n",
				fake_path);
		printf("[anti_spam]: the plugin %s is not loaded\n", fake_path);
		dlclose(handle);
	...
	if (NULL == plib) {
		printf("[anti_spam]: fail to allocate memory for %s\n", fake_path);
		printf("[anti_spam]: the plugin %s is not loaded\n", fake_path);
		dlclose(handle);
	...
	/* for all plugins, there's should be a read-write lock for controlling 
	   its modification. ervery time, the anti-spam function is invoked, 
	...
	*/
	...
	if (FALSE == func(PLUGIN_INIT, (void**) two_server_funcs)) {
		printf("[anti_spam]: error to excute plugin's init function "
				"in %s\n", fake_path);
		printf("[anti_spam]: the plugin %s is not loaded\n", fake_path);
		/*
		the lib node will automatically removed from libs list in 
		anti_spamm_unload_library function
		*/
		anti_spam_unload_library(fake_path);
		g_cur_lib = NULL;
...
/*
 *	register a certain statistic in the anti-spam module
 *	@param
...
/*
 *	  register a certain type of filter in the anti-spam module
 *	  @param
...
static BOOL anti_spam_register_filter(char* type, FILTER_FUNCTION func)
{
	...
	if (NULL == pfilter) {
		printf("[anti_spam]: fail to allocate filter node in register"
				" filter\n");
		return FALSE;
	}
	...
	if (NULL == ptype) {
		...
		*/
		...
		if (NULL != pnode) {
			printf("[anti_spam]: function is already registered \n");
			free(pfilter);
			return FALSE;
		}
...
/*
 *	register a certain auditor in the anti-spam module
 *	@param
...
/*
 *	register a certain judge in the anti-spam module
 *	@param
...
/*
 *	unregister a statistic in the anti-spam module
 *	@param
...
/*
 *	  unregister a certain type of filter in the anti-spam module	 
 *	  @param
...
/*
 *	unregister a auditor in the anti-spam module
 *	@param
...
/*
 *	unregister a judge in the anti-spam module
 *	@param
 ...
 *		  PLUGIN_NOT_FOUND	  plugin is not found in anti-spam module
...
{	 
	...
	if(0 != double_list_get_nodes_num(&plib->list_filter)) {
		...
		for (pnode=double_list_get_head(&plib->list_filter); NULL!=pnode; 
			...
			ptype = (TYPE_NODE*)str_hash_query(g_hash_type, tmp_buff);
			if (NULL == ptype) {
				printf("[anti_spam]: fatal error when"
					" unload the anti-spam plugin\n");
...
int anti_spam_reload_library(const char* path)
{
	...
	retval	= anti_spam_unload_library(path);
	...
	retval	= anti_spam_load_library(old_path);
...
/*
 *	  let plugins' "judges" to judge wheter the mail is spam
 *	  @param
 ...
 */
int anti_spam_pass_judges(SMTP_CONTEXT* pcontext, char *reason, int length)
{
	...
	for (pnode=double_list_get_head(&g_list_judge); NULL!=pnode; 
		 pnode= double_list_get_after(&g_list_judge, pnode)) {
		anti_spam_reset_envelop_files(penvelop);
		...
		if (MESSAGE_ACCEPT != judge_result &&
			...
			printf("[anti_spam]: return value error in judge function "
...
/*
 *	  let plugins' auditors to judge wheter the mail is spam
 *	  @param
 ...
 *		  MESSAGE_RETRYING it may be spam, need retry
 */
int anti_spam_pass_auditors(SMTP_CONTEXT* pcontext,
	char *reason, int length)
{
	...
	for (pnode=double_list_get_head(&g_list_auditor); NULL!=pnode; 
		 pnode= double_list_get_after(&g_list_auditor, pnode)) {
		anti_spam_reset_envelop_files(&pcontext->mail.envelop);
		anti_spam_reset_head_files(&pcontext->mail.head);
		...
		if (MESSAGE_ACCEPT != audit_result &&
			...
			printf("[anti_spam]: return value error in auditor function "
 ...
 */
void anti_spam_inform_filters(const char *type,
	SMTP_CONTEXT *pcontext, int action, int block_ID)
{
	DOUBLE_LIST_NODE *pnode;
	TYPE_NODE		 *ptype;
...
/*
 *	  let filters of "type" judge whether it is spam
 *	  @param
 *		  type					  indicate the type to filter
	...
	if (NULL != ptype) {
		for (pnode=double_list_get_head(&ptype->list); NULL!=pnode;
			...
			if (MESSAGE_ACCEPT != result &&
				...
				printf("[anti_spam]: return value error in filter function "
					"%p!!!\n", ((FILTER_NODE*)(pnode->pdata))->filter_func);
				continue;
			}
			if (MESSAGE_ACCEPT != result) {
...
/*
 *	  let plugins' statistics to judge wether the mail is spam
 *	  @param
 *		  pmail [in]	   some information about the mail
 *		  pconnection [in] some information about connection
 *		  reason [out]	   buffer for retieving the reason
	...
	int length)
{
	...
	for (pnode=double_list_get_head(&g_list_statistic); NULL!=pnode; 
		...
		anti_spam_reset_envelop_files(&pcontext->mail.envelop);
		anti_spam_reset_head_files(&pcontext->mail.head);
		anti_spam_reset_body_files(&pcontext->mail.body);
		...
		if (MESSAGE_ACCEPT != statistic_result &&
			...
			printf("[anti_spam]: return value error in statistic function "
...
int anti_spam_stop()
{
	...
	if (NULL == pallocator) {
		debug_info("[anti_spam]: fail to init allocator for stack in" 
				"anti_spam_stop\n");
		return -1;
	}
	...
	while (FALSE == vstack_is_empty(&stack)) {
		anti_spam_unload_library(vstack_get_top(&stack));
		vstack_pop(&stack);
	}
	vstack_free(&stack);
...
/*
 *	  anti-spam modile's destruct function
 */
void anti_spam_free()
{
	g_init_path[0] = '\0';
...
static void* anti_spam_queryservice(char *service)
{
	...
	if (strcmp(service, "register_filter") == 0) {
		return anti_spam_register_filter;
	}
	if (strcmp(service, "unregister_filter") == 0) {
		return anti_spam_unregister_filter;
	}
	if (strcmp(service, "register_auditor") == 0) {
		return anti_spam_register_auditor;
	}
	if (strcmp(service, "unregister_auditor") == 0) {
		return anti_spam_unregister_auditor;
	}
	if (strcmp(service, "register_judge") == 0) {
		return anti_spam_register_judge;
	}
	if (strcmp(service, "unregister_judge") == 0) {
		return anti_spam_unregister_judge;
	}
	if (strcmp(service, "register_statistic") == 0) {
		return anti_spam_register_statistic;
	}
	if (strcmp(service, "unregister_statistic") == 0) {
		return anti_spam_unregister_statistic;
	}
	if (strcmp(service, "register_talk") == 0) {
		return anti_spam_register_talk;
	}
	if (strcmp(service, "unregister_talk") == 0) {
		return anti_spam_unregister_talk;
	}
	...
	if (strcmp(service, "set_extra_value") == 0) {
		return anti_spam_set_extra_value;
	}
	if (strcmp(service, "mark_context_spam") == 0) {
		return anti_spam_mark_context_spam;
	}
	if (strcmp(service, "is_need_auth") == 0) {
		return anti_spam_is_need_auth;
	}
	if (strcmp(service, "is_domainlist_valid") == 0) {
		return anti_spam_is_domainlist_valid;
	}
	if (strcmp(service, "get_default_domain") == 0) {
		return anti_spam_get_default_domain;
	}
	if (strcmp(service, "get_plugin_name") == 0) {
		return anti_spam_get_plugin_name;
	}
	if (strcmp(service, "get_config_path") == 0) {
		return anti_spam_get_config_path;
	}
	if (strcmp(service, "get_data_path") == 0) {
		return anti_spam_get_data_path;
	}
	if (strcmp(service, "get_context_num") == 0) {
		return anti_spam_get_context_num;
	}
	...
	if (NULL == pservice) {
		debug_info("[anti-spam]: fail to allocate memory for service node");
		service_release(service, g_cur_lib->file_name);
		return NULL;
	}
	pservice->service_name = malloc(strlen(service) + 1);
	if (NULL == pservice->service_name) {
		debug_info("[anti-spam]: fail to allocate memory for service name");
		service_release(service, g_cur_lib->file_name);
		free(pservice);
		return NULL;
