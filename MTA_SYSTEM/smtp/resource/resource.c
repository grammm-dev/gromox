...
{
	if (NULL == g_config_file) {
		debug_info("[resource]: error!!! config file not"
			" init or init fail but it is now being used");
		return FALSE;
	}
...
BOOL resource_get_integer(int key, int* value)
{
	...
	if (NULL == pvalue) {
		debug_info("[resource]: no value map to"
			" the key in resource_get_integer");
		return FALSE;
	}
	*value = atoi(pvalue);
	return TRUE;
...
const char* resource_get_string(int key)
{
	...
	if (NULL == pvalue) {
		debug_info("[resource]: no value map to"
			" the key in resource_get_string");
		return NULL;
	}
	return pvalue;
}
...
static int resource_construct_smtp_table(SMTP_ERROR_CODE **pptable)
{
	...
	if (NULL == (file_ptr = fopen(filename, "r"))) {
		printf("[resource]: can not open smtp "
			"error table file  %s\n", filename);
		return -1;
	}
	...
	if (NULL == code_table) {
		printf("[resource]: fail to allocate "
			"memory for smtp return code table\n");
		fclose(file_ptr);
		return -1;
	}
	...
	for (total = 0; fgets(line, MAX_FILE_LINE_LEN, file_ptr); total++) {
		...
		if (len <= 0 || len > sizeof(code) - 1) {
			printf("[resource]: invalid native code format, file:"
		...
		if ((native_code = atoi(code)) <= 0) {
			printf("[resource]: invalid native code, file: %s"
				" line: %d, %s\n", filename, total + 1, line);
			continue;
		}
		...
		if (len <= 0 || len > MAX_FILE_LINE_LEN - 1) {
			printf("[resource]: invalid native comment, file: "
				"%s line: %d, %s\n", filename, total + 1, line);
			continue;
		}
		...
		if ((index = resource_find_smtp_code_index(native_code)) < 0) {
			printf("[resource]: no such native code, file: %s"
				" line: %d, %s\n", filename, total + 1, line);
			continue;
		}

		if (-1 != code_table[index].code) {
			printf("[resource]: the error code has already been defined,"
				" file: %s line: %d, %s\n", filename, total + 1, line);
			continue;

		}

		if (resource_parse_smtp_line(
			code_table[index].comment, buf, len) < 0) {
			printf("[resource]: invalid smtp code format, file:"
				" %s line: %d, %s", filename, total + 1, line);
			continue;
		}
...
#define SECOND_PART     2
	...
	if (NULL == g_error_code_table ||
		-1 == g_error_code_table[code_type].code) {
