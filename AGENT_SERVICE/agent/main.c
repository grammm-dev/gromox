...
void start_adac()
{
	...
	char temp_path[256];
	char *args[] = {"./adac", "../config/agent.cfg", NULL};

	sprintf(temp_path, "%s/adac", AGENT_MAIN_DIR);
	...
	if (g_adac_pid < 0) {
		...
		while (0 == g_notify_stop) {
			...
			if (0 == g_supervised_process) {
				chdir(AGENT_MAIN_DIR);
				if (execve("./adac", args, NULL) == -1) {
					exit(EXIT_FAILURE);
				}
...
void start_asession()
{
	...
	struct stat node_stat;
	char temp_path[256];
	char *args[] = {"./asession", "../config/agent.cfg", NULL};

	sprintf(temp_path, "%s/asession", AGENT_MAIN_DIR);
	if (0 != stat(temp_path, &node_stat)) {
		g_asession_pid = -1;
		return;
	}
...
void start_alock()
{
	...
	struct stat node_stat;
	char temp_path[256];
	char *args[] = {"./alock", "../config/agent.cfg", NULL};

	sprintf(temp_path, "%s/alock", AGENT_MAIN_DIR);
	if (0 != stat(temp_path, &node_stat)) {
		g_alock_pid = -1;
		return;
	}
...
void start_asensor()
{
	...
	struct stat node_stat;
	char temp_path[256];
	char *args[] = {"./asensor", "../config/agent.cfg", NULL};

	sprintf(temp_path, "%s/asensor", AGENT_MAIN_DIR);
	if (0 != stat(temp_path, &node_stat)) {
		g_asensor_pid = -1;
		return;
	}
...
void start_amidb()
{
	...
	struct stat node_stat;
	char temp_path[256];
	char *args[] = {"./amidb", "../config/agent.cfg", NULL};

	sprintf(temp_path, "%s/amidb", AGENT_MAIN_DIR);
	if (0 != stat(temp_path, &node_stat)) {
		g_amidb_pid = -1;
		return;
	}
...
void stop_service()
{
	...
	pid = atoi(str);
	if (0 == pid) {
		printf("fail to get agent's pid\n");
		exit(EXIT_FAILURE);
	}
	kill(pid, SIGTERM);
	exit(EXIT_SUCCESS);
...
int main(int argc, char **argv)
{
	...
	sprintf(AGENT_MAIN_DIR, "%s/bin", argv[1]);
