	...
	EXT_PULL *pext, REQUEST_PAYLOAD *ppayload)
{
	...
	if (0 == tmp_byte) {
		ppayload->setpropvals.pnamespace = NULL;
	} else {
		if (EXT_ERR_SUCCESS != ext_buffer_pull_string(
			pext, &ppayload->setpropvals.pnamespace)) {
			return FALSE;
...
static BOOL rpc_ext_push_getaddressbook_response(
	EXT_PUSH *pext, const RESPONSE_PAYLOAD *ppayload)
{
	if (EXT_ERR_SUCCESS != ext_buffer_push_string(
		pext, ppayload->getaddressbook.ab_address)) {
		return FALSE;	
	}
	return TRUE;
}

static BOOL rpc_ext_push_getabhierarchy_response(
	EXT_PUSH *pext, const RESPONSE_PAYLOAD *ppayload)
{
	return rpc_ext_push_dac_vset(pext, &ppayload->getabhierarchy.set);
}

static BOOL rpc_ext_pull_getabcontent_request(
	EXT_PULL *pext, REQUEST_PAYLOAD *ppayload)
{
	if (EXT_ERR_SUCCESS != ext_buffer_pull_uint32(
		pext, &ppayload->getabcontent.start_pos)) {
		return FALSE;	
	}
	if (EXT_ERR_SUCCESS != ext_buffer_pull_uint16(
		pext, &ppayload->getabcontent.count)) {
		return FALSE;	
	}
	return TRUE;
}

static BOOL rpc_ext_push_getabcontent_response(
	EXT_PUSH *pext, const RESPONSE_PAYLOAD *ppayload)
{
	return rpc_ext_push_dac_vset(pext, &ppayload->getabcontent.set);
}

static BOOL rpc_ext_pull_restrictabcontent_request(
	EXT_PULL *pext, REQUEST_PAYLOAD *ppayload)
{
	if (FALSE == rpc_ext_pull_dac_res(pext,
		&ppayload->restrictabcontent.restrict)) {
		return FALSE;	
	}
	if (EXT_ERR_SUCCESS != ext_buffer_pull_uint16(
		pext, &ppayload->restrictabcontent.max_count)) {
		return FALSE;	
	}
	return TRUE;
}

static BOOL rpc_ext_push_restrictabcontent_response(
	EXT_PUSH *pext, const RESPONSE_PAYLOAD *ppayload)
{
	return rpc_ext_push_dac_vset(pext, &ppayload->restrictabcontent.set);
}

static BOOL rpc_ext_push_getexaddress_response(
	EXT_PUSH *pext, const RESPONSE_PAYLOAD *ppayload)
{
	if (EXT_ERR_SUCCESS != ext_buffer_push_string(
		pext, ppayload->getexaddress.ex_address)) {
		return FALSE;	
	}
	return TRUE;
}

static BOOL rpc_ext_push_getexaddresstype_response(
	EXT_PUSH *pext, const RESPONSE_PAYLOAD *ppayload)
{
	if (EXT_ERR_SUCCESS != ext_buffer_push_uint8(
		pext, ppayload->getexaddresstype.address_type)) {
		return FALSE;	
	}
	return TRUE;
}

static BOOL rpc_ext_pull_checkmlistinclude_request(
	EXT_PULL *pext, REQUEST_PAYLOAD *ppayload)
{
	if (EXT_ERR_SUCCESS != ext_buffer_pull_string(pext,
...
static BOOL rpc_ext_push_checkmlistinclude_response(
	EXT_PUSH *pext, const RESPONSE_PAYLOAD *ppayload)
{
	if (EXT_ERR_SUCCESS != ext_buffer_push_bool(pext,
	...
	RPC_REQUEST *prequest)
{
	...
	case CALL_ID_GETABCONTENT:
		return rpc_ext_pull_getabcontent_request(
					&ext_pull, &prequest->payload);
	case CALL_ID_RESTRICTABCONTENT:
		return rpc_ext_pull_restrictabcontent_request(
						&ext_pull, &prequest->payload);
	case CALL_ID_GETEXADDRESS:
	case CALL_ID_GETEXADDRESSTYPE:
		return TRUE;
	case CALL_ID_CHECKMLISTINCLUDE:
		return rpc_ext_pull_checkmlistinclude_request(
						&ext_pull, &prequest->payload);
	...
	BINARY *pbin_out)
{
	...
	case CALL_ID_STATFILE:
		...
		break;
	case CALL_ID_GETADDRESSBOOK:
		b_result = rpc_ext_push_getaddressbook_response(
						&ext_push, &presponse->payload);
		break;
	case CALL_ID_GETABHIERARCHY:
		b_result = rpc_ext_push_getabhierarchy_response(
						&ext_push, &presponse->payload);
		break;
	case CALL_ID_GETABCONTENT:
		b_result = rpc_ext_push_getabcontent_response(
						&ext_push, &presponse->payload);
		break;
	case CALL_ID_RESTRICTABCONTENT:
		b_result = rpc_ext_push_restrictabcontent_response(
							&ext_push, &presponse->payload);
		break;
	case CALL_ID_GETEXADDRESS:
		b_result = rpc_ext_push_getexaddress_response(
						&ext_push, &presponse->payload);
		break;
	case CALL_ID_GETEXADDRESSTYPE:
		b_result = rpc_ext_push_getexaddresstype_response(
							&ext_push, &presponse->payload);
		break;
	case CALL_ID_CHECKMLISTINCLUDE:
		b_result = rpc_ext_push_checkmlistinclude_response(
							&ext_push, &presponse->payload);
		break;
	case CALL_ID_CHECKROW:
		b_result = TRUE;
